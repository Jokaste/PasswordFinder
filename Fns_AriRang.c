...
static const unsigned int K256[16] = {0x517cc1b7, 0x76517cc1, 0xbd76517c, 0x2dbd7651,
...
{   
    ...
    for (i = 0; i < 256; ++i)
    {
        ...
        t = ((unsigned int)ff_mult(2, p)) | ((unsigned int)p <<  8) | ((unsigned int)p << 16) | ((unsigned int)ff_mult(3, p) << 24);
...
//
// PARAMETERS     : state - a structure that holds the AriRang_hashState information
//
// RETURNS        : void
//
///////////////////////////////////////////////////////////////////////////////////////////////////

void Arirang_Compression256(AriRang_hashState *state)
{
	...
	// Counter Addition
	state->workingvar[0] ^= ((unsigned int*)state->counter)[1];
	state->workingvar[4] ^= ((unsigned int*)state->counter)[0];
...
void Arirang_Compression512(AriRang_hashState *state)
{
	...
	// Message Scheduling
	for (i = 0; i < 16; i++)
		W[i] = (QWORD)(GetData(((unsigned int*)state->block)[2*i+1])) | ((QWORD)(GetData(((unsigned int*)state->block)[2*i])) << 32); 
...
// FUNCTION NAME  : Init
//
// DESCRIPTION    : Initialize a AriRang_hashState with the intended hash length of particular instantiation.
//
// PARAMETERS     : state - a structure that holds the AriRang_hashState information
...
int AriRang_Init(AriRang_hashState *state, int hashbitlen)
{
...
int AriRang_Update(AriRang_hashState *state,unsigned char *data,unsigned long long databitlen)
{
...
int AriRang_Final(AriRang_hashState *state, BYTE *hashval)
{
	unsigned int i, dwIndex;
	unsigned int temp=(state->blocklen >> 3);
	QWORD count[2];
	...
	count[0] = ENDIAN_REVERSE_DWORD(((unsigned int*)count)[1])| ((QWORD)(ENDIAN_REVERSE_DWORD(((unsigned int*)count)[0])) << 32);
	count[1] = ENDIAN_REVERSE_DWORD(((unsigned int*)count)[3])| ((QWORD)(ENDIAN_REVERSE_DWORD(((unsigned int*)count)[2])) << 32);
#endif
	...
	if(state->hashbitlen <257)
		for (i = 0; i < (unsigned int) (state->hashbitlen >> 3); i += 4)
			BIG_D2B((state->workingvar)[(i*2) / 8], &(hashval[i]));
	else
		for (i = 0; i < (unsigned int) (state->hashbitlen >> 3); i += 8)
...
//
///////////////////////////////////////////////////////////////////////////////////////////////////
int AriRang_Hash(int hashbitlen,unsigned char *data,unsigned long long *databitlen,unsigned char *hashval)
{

	AriRang_hashState State;
	BitSequence *UpdatedData;	
	DataLength UpdatedDataLengthbit=0x100000000;
	register	int i,j;
	
	UpdatedData=(unsigned char *) data;
	State.remainderbit = databitlen[0] & 7;

	AriRang_Init(&State, hashbitlen);

	i=0,j=0;
	while(((unsigned int) j < (unsigned int) databitlen[1]) && ((unsigned int) i < ((unsigned int*)databitlen)[1])){
		AriRang_Update(&State, UpdatedData, UpdatedDataLengthbit);
