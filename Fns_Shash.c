...
HashReturn SHash_Init(hashState *state, int hbitlen) {
...
HashReturn SHash_Update(hashState *state, const BitSequence *data, DataLength databitlen)
{
...
HashReturn SHash_Final(hashState *state, BitSequence *hashval){
   int i;
   int currentbyte = 0;
...
HashReturn SHash(int hashbitlen, const BitSequence *data, DataLength databitlen, BitSequence *hashval)
{
	hashState state;
	if(SHash_Init(&state, hashbitlen) == BAD_HASHBITLEN){/* initializes state and checks to make sure hashbitlen is valid */
		return BAD_HASHBITLEN;
	}
	SHash_Update(&state, data, databitlen); /* handles the bulk of the compresses */
	SHash_Final(&state, hashval); /* handles leftover bits and calculates hash value */
	return SUCCESS;
}
